package github.gzhc.cmdnlist.utils;
...
public final class Reflection {
        ...
        for (int length = (declaredFields = target.getDeclaredFields()).length, i = 0; i < length; ++i) {
            ...
            if ((name == null || field.getName().equals(name)) && fieldType.isAssignableFrom(field.getType()) && index-- <= 0) {
                ...
                return new FieldAccessor<T>() {
                    ...
                    @Override
                    public void set(final Object target, final Float value) {
                        try {
    ...
    static <T> FieldAccessor<T> getField(final Class<?> target, final String name) {
        Field[] declaredFields;
        for (int length = (declaredFields = target.getDeclaredFields()).length, i = 0; i < length; ++i) {
            final Field field = declaredFields[i];
    ...
    public interface FieldAccessor<T> {
        ...
        void set(final Object p0, final Float p1);
